...
<div class="user-profile-search element-to-toggle" data-toggle-element="form-search">
  ...
  <?php if (!$adminModule->bypassWorkflow) { ?>
    ...
    <div class="col-sm-6 col-lg-3">
    <?=
      $form->field($model, 'validato_almeno_una_volta')->dropDownList(Html::getBooleanFieldsValues(),
        ['prompt' => AmosAdmin::t('amosadmin', 'Select/Choose') . '...', 'disabled' => false])
  ...
  ?>
    ...
    <div class="col-sm-6 col-lg-3">
    <?=
      $form->field($model, 'isFacilitator')->dropDownList(Html::getBooleanFieldsValues(),
        ['prompt' => AmosAdmin::t('amosadmin', 'Select/Choose') . '...', 'disabled' => false])
  ...
  <?php
    $moduleNameOrganizations = $adminModule->getOrganizationModuleName();
    if (!empty(\Yii::$app->getModule($moduleNameOrganizations))) {
  ?>
    <div class="col-sm-6 col-lg-3">
    <?=
      $form->field($model, 'isOperatingReferent')->dropDownList(Html::getBooleanFieldsValues(),
        ['prompt' => AmosAdmin::t('amosadmin', 'Select/Choose') . '...', 'disabled' => false])
